...
popup_text1 <- dplyr::select(grizzdata_full, gbpu_name, rank_label, 
                  threat_class, adults, isolation, trend) %>% 
  rename("Population Name" = gbpu_name,
         "Conservation Concern" = rank_label,
...
palette1a <- colorFactor(palette = 'viridis', grizzdata_full$rank_label, reverse = TRUE)
...
grizzmap %>% 
  addPolygons(data = grizzdata_full, group = "Conservation Status",
              ...
              highlightOptions = highlightOptions( # Highlight interaction for mouse hover
                ...
                color = "yellow",
                bringToFront = T)) %>%
  addLegend("bottomright", pal = palette1a, values = grizzdata_full$rank_label,
            title = "Conservation Concern", na.label = "Extirpated",
            opacity = 1) 
```
