...
grizz_summary <- dplyr::select(grizzdata_full, gbpu_name, status, est_pop__1, 
                               iso, threat_class) %>%
  rename("Population Name" = gbpu_name,
         "Overall Status" = status,
         "Population Size" = est_pop__1,
         "Isolation" = iso,
...
palette2 <- colorNumeric("PuBuGn", grizzdata_full$est_pop_de,
                        na.color = "#808080")
...
grizzmap %>%
  addPolygons(data = grizzdata_full, #group = "Population Estimate",
              ...
              fillOpacity = 0.6,
              fillColor = ~palette2(grizzdata_full$est_pop_de),
              label = plotlabs,
              labelOptions = labelOptions(direction = "auto", textsize = "12px"),
              popup = popups,
              popupOptions = popup_options,
              highlight = highlightOptions( # Highlight interaction for mouse hover
                ...
                color = "yellow",
                bringToFront = T)) %>%
  addLegend("bottomright", pal = palette2, values = grizzdata_full$est_pop_de,
