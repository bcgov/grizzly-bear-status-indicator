...
* Grizzly bears are an important part of the British Columbia landscape. They play a key role in maintaining healthy ecosystems, and represent an important part of the culture of First Nations People living in B.C.

* Grizzly bears are known as “umbrella” species. This means landscapes that support healthy Grizzly bear populations are able to sustain many other species.

* For management purposes, grizzly bears are divided into 55 Grizzly Bear Population Units (GBPUs) across B.C. These range in size from 2,670 km^2^ to 49,578 km^2^. 

* Using GBPUs helps us track grizzly bear abundance and stability, and identifies localised conservation concerns. 

* Each GBPU is ranked from lowest conservation concern (M5) to highest conservation concern (M1). These ranking are based on international methods used by NatureServe^2^ and IUCN. Population Unit rankings are determined by 1) population size and isolation, 2) population trend and 3) level of threat to bears or bear habitat. Ranks are deterined using the latest available data and expert opinion. 

* Seven catergories are considered in determining threat. These include; Agriculture, Biological Use, Climate Change, Energy, Human intrusion, Residential, Transportation. See below for detailed methodology ^1^.  
...
```{r cons_map, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE, results = 'asis', out.width='0.99\\linewidth', fig.align = 'center'}
...
``` {r pop map, warning=FALSE, echo = FALSE, error = FALSE, message = FALSE, results = 'asis', out.width='0.99\\linewidth', fig.align = 'center'}
...
``` {r threat map, warning=FALSE, echo = FALSE, error = FALSE, message = FALSE, results = 'asis', out.width='0.99\\linewidth', fig.align = 'center'}
...
Threat classifications are based on three factors; 1) Population and Isolation, 2) population trend and 3) overall threats. The influence of these three factors on the overall threat rank are show below. 
