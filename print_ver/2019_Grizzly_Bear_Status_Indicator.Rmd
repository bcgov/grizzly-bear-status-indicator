...
output: word_document #envreportutils.internal::print_ver
params:
  draft: TRUE
header-includes:
...
* **Grizzly bears are an important part of the British Columbia landscape.** 
  ...
  + Grizzly bears are listed as a ‘Species of Concern’ under federal legislation^1^ and ranked as S3 (Vulnerable Uncertain) provincially^2^. 

* **Grizzly bears are divided into 55 Grizzly Bear Population Units (GBPUs) across B.C.** These units range from 2,670 km^2^ to 49,578 km^2^. They enable managers to identify local conservation concerns, track grizzly bear population trends and apply specific management practices.
...
```{r radar_key, echo = FALSE, message=FALSE, fig.align='right', fig.cap = "Contribution of criteria (Trend, Threat, Population/Isolation) to the overall GBPU ranking. Each arm represents the relative contribution of each factor. For example: Population/Isolation (0-4), Threat (0-2) and Trend (1). Coloured areas represent the influence of each factor to the overall ranking. For example greater area coloured represents greather threat to grizzly GBPU and ultimately increase in ranking from low to extreme." , out.width='0.30\\linewidth'}
